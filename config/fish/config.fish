# $Arch: config.fish,v 1.011 2017/08/10 02:02:12 kyau Exp $
...
if status --is-interactive
  ...
  # `set -gx` replaced `setenv` in fish 2.6+
	...
	function __autostart_ssh
		if test -n $SSH_CLIENT -a -n $SSH_TTY
	...
	set -l agentpid (ps -ef | grep $SSH_AGENT_PID | grep "/usr/bin/ssh-agent" | awk '!/grep/')
	if test -z $agentpid
		__autostart_ssh
	end
	# }}}
